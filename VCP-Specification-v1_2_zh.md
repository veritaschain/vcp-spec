...
VCP v1.2是**协议兼容 / 认证更严格**的更新。**零破坏性变更**：所有v1.0和v1.1的事件数据与v1.2验证者完全互操作。v1.2收紧了VC-Certified要求（尤其是VCP-RECOVERY约束），并为GDPR擦除、SCITT互操作和多方审计链添加了可选功能。

> v1.2所有变更的完整规范性规格，记载于本目录中作为**规范性附件（Annex）**随附的配套文档 **[VSO-SPEC-CHANGE-001](VSO-SPEC-CHANGE-001.md)**（"VCP v1.2变更提案"）。本节为其摘要。若本摘要与附件细节不一致，以附件为准。

### 变更分类

| 类别 | 数量 | 影响 |
|------|------|------|
| 规范性（REQUIRED） | 7 | 收紧VC-Certified要求 |
| 参考性（RECOMMENDED） | 2 | 文档 / 指南 |
| 破坏性变更 | 0 | 保持完全向后兼容 |
...
| # | 变更 | 类型 | 模块 / 章节 | Annex |
|---|------|------|------------|-------|
| 1 | VCP-RECOVERY约束收紧（SKIP/REBUILD/MERGE/CHECKPOINT边界、紧急覆盖） | 规范性 | VCP-RECOVERY | §1 |
| 2 | 外部锚定区块链选择标准 | 规范性 | 第3层（§6.3） | §2 |
| 3 | ERASURE事件类型（GDPR第17条 / 加密粉碎） | 规范性 | VCP-PRIVACY、事件注册表（§3.2.2） | §3 |
| 4 | 版本兼容性矩阵 | 参考性 | 迁移（§10.5） | §4 |
| 5 | SCITT对齐字段（COSE Receipts、透明性服务） | 规范性（可选） | VCP-CORE / §6 | §5 |
| 6 | 延迟预算澄清 | 参考性 | 层级（§2） / §7 | §6 |
| 7 | 锚定目标连续性计划 | 规范性 | 第3层（§6.3） | §7 |
| 8 | 参考实现基准 | 参考性 | §7 | §8 |
| 9 | 多参与方链链接（跨方VCP-XREF） | 规范性（使用时） | VCP-XREF（§5.6） | §9 |

> **注**: 多参与方链链接（#9）被分类为规范性，因为使用时会施加MUST级别的要求。但其采用本身为OPTIONAL。

### 认证与算法说明

- **VC-Certified v1.2**额外要求符合收紧的VCP-RECOVERY约束（Annex §1）和文档化的锚定连续性计划（Annex §7）。v1.0/v1.1认证的实现保持协议兼容，但v1.2认证需满足这些约束。
- **抗量子签名**（`DILITHIUM2`、`FALCON512`）在v1.2中从*FUTURE*（保留）进阶为*EXPERIMENTAL*。可用于测试和混合（经典＋PQC）部署，但**尚非认证要求**。`Ed25519`仍为DEFAULT。参见§1.5.1和附录E。
...
- **实验性（v1.2新增）**: 抗量子算法（DILITHIUM2 / ML-DSA、FALCON512 / FN-DSA）— 可用于测试和混合（经典＋PQC）部署。尚非认证要求
- **迁移路径**: 自动算法升级功能（PQC迁移路线图参见附录E）
...
| **DILITHIUM2** | ML-DSA (FIPS 204) | 抗量子（NIST Level 2） | EXPERIMENTAL |
| **FALCON512** | FN-DSA (FIPS 206 draft) | 抗量子（NIST Level 1） | EXPERIMENTAL |
...
VCP v1.1引入标准化的错误事件类型，以确保跨实现的一致错误记录。

| EventType | 类别 | 描述 | 严重性 |
|-----------|------|------|--------|
| **ERR_CONN** | 连接 | 连接失败（经纪商、交易所、数据源） | CRITICAL |
| **ERR_AUTH** | 认证 | 认证/授权失败 | CRITICAL |
| **ERR_TIMEOUT** | 超时 | 操作超时（订单、查询、心跳） | WARNING |
| **ERR_REJECT** | 拒绝 | 对手方拒绝订单/请求 | WARNING |
| **ERR_PARSE** | 数据 | 消息解析或验证失败 | WARNING |
| **ERR_SYNC** | 同步 | 时钟同步丢失、序列间隙检测 | WARNING |
| **ERR_RISK** | 风险 | 风险限制违规、头寸限制超出 | CRITICAL |
| **ERR_SYSTEM** | 系统 | 内部系统错误、资源耗尽 | CRITICAL |
| **ERR_RECOVER** | 恢复 | 恢复操作启动（重连、重同步） | INFO |
...
```json
{
  ...
  "ErrorDetails": {
    "ErrorCode": "string",           // 实现特定代码
    "ErrorMessage": "string",        // 人类可读描述
    "Severity": "enum",              // CRITICAL | WARNING | INFO
    "AffectedComponent": "string",   // 例如: "broker-gateway", "price-feed"
    "RecoveryAction": "string",      // OPTIONAL: 采取或建议的操作
    "CorrelatedEventID": "uuid"      // OPTIONAL: 导致错误的相关事件
  }
}
```
...
| 要求 | 级别 | 注意 |
|------|------|------|
| 错误事件记录 | REQUIRED | 所有层必须记录错误事件 |
| ErrorDetails.Severity | REQUIRED | 必须分类所有错误 |
| ErrorDetails.ErrorMessage | REQUIRED | 人类可读描述 |
| ErrorDetails.RecoveryAction | RECOMMENDED | 有助于事件分析 |
| CorrelatedEventID | OPTIONAL | 链接到触发事件 |

> **实现说明**: 错误事件遵循与所有VCP事件相同的完整性要求（EventHash、Merkle包含、锚定）。错误事件不得从锚定批次中过滤掉（MUST NOT）。

#### 3.2.2 ERASURE事件类型（v1.2新增）
...
v1.2**不引入破坏性变更**。现有v1.1实现无需修改即可与v1.2验证者互操作，所有v1.1事件仍为有效的v1.2事件。要获得**VC-Certified v1.2**，实现还需满足：

1. **采用VCP-RECOVERY约束**（REQUIRED）— 强制执行按层级的SKIP/REBUILD/MERGE/CHECKPOINT边界和紧急覆盖批准（Annex §1）。
2. **文档化锚定连续性**（REQUIRED）— 声明后备锚定目标和故障转移程序（Annex §7）。
3. **可选采用**: ERASURE（GDPR）、SCITT对齐字段、多参与方XREF（Annex §3、§5、§9）。
...
| 生产者 | 消费者 | 兼容性 | 注意 |
|--------|--------|--------|------|
| v1.0 | v1.2 | 完全 | v1.2接受所有v1.0事件 |
| v1.1 | v1.2 | 完全 | v1.2接受所有v1.1事件 |
| v1.2 | v1.0 | 部分 | ERASURE和新字段被忽略 |
| v1.2 | v1.1 | 部分 | ERASURE事件对v1.1未知 |
| v1.2 | v1.2 | 完全 | — |
...
| 证明 | 验证者 | 兼容性 | 注意 |
|------|--------|--------|------|
| v1.0 / v1.1的证明 | v1.2验证者 | 完全 | 所有旧证明可验证 |
| v1.2的证明 | v1.0 / v1.1验证者 | 部分 | 新证明类型优雅失败 |

> 完整矩阵（包含认证兼容性和算法弃用时间线）参见Annex §4。
...
VCP被设计为**边车**组件，与现有交易系统并行运行，无需修改核心交易逻辑或基础设施。
...
- **优点**: 最简单的集成，单一部署
- **缺点**: 增加进程内依赖
- **适用**: Silver层，MT4/MT5 EA集成

##### 模式C: 消息队列抓取

```
交易系统 ──> [Kafka/Redis] ──> 经纪商
                        │
                        └──> VCP消费者 ──> 审计跟踪
```
...
- **优点**: 解耦，可扩展，重放能力
- **缺点**: 需要消息基础设施
- **适用**: Gold/Platinum层，机构系统

#### F.3 特定平台集成

| 平台 | 集成方法 | VCP组件 |
|------|---------|---------|
| **MT4/MT5** | DLL + EA钩子 | vcp-mql-bridge |
| **cTrader** | cBot插件 | vcp-ctrader-plugin |
| **FIX引擎** | FIX适配器 | vcp-fix-sidecar |
| **自定义算法** | REST/gRPC API | vcp-core-py / vcp-core-cpp |

#### F.4 关键设计原则

| 原则 | 描述 |
|------|------|
| **非侵入性** | 不更改现有交易逻辑或数据库模式 |
| **故障安全** | VCP故障不得影响交易操作 |
| **异步优先** | 尽可能异步捕获事件 |
| **幂等性** | 重复事件处理必须安全 |
| **可恢复** | 支持故障后重放和间隙填充 |

> **关键**: VCP边车故障不得导致交易系统故障。实现断路器和回退模式。
...
- **draft-ietf-scitt-architecture**: SCITT — Supply Chain Integrity, Transparency and Trust（IETF，制定中）
- **draft-ietf-cose-merkle-tree-proofs**: COSE Receipts（IETF，制定中）
- **draft-kamimura-scitt-vcp**: 金融交易审计跟踪的SCITT配置文件 — VCP（IETF Internet-Draft，个人提交，无IETF正式地位）

### 法规
- **MiFID II / MiFIR**: 金融工具市场指令 / 法规（含2024年MiFID III / MiFIR II审查）
- **RTS 6 / RTS 24 / RTS 25**: 监管技术标准（算法交易组织要求、订单记录保存、业务时钟同步）
- **CAT Rule 613**: 综合审计跟踪
- **SEC Rule 17a-4 / 18a-6**: 电子记录保存（WORM或审计跟踪替代方案，2022年修订）
- **GDPR**: 通用数据保护条例（含第17条擦除权）
- **EU AI Act**: 法规 (EU) 2024/1689（含第12条记录保存）
...
### VSO规范性附件（Annex）
- **VSO-SPEC-CHANGE-001**: VCP v1.2变更提案 — v1.2所有变更的完整规范性规格（[VSO-SPEC-CHANGE-001.md](VSO-SPEC-CHANGE-001.md)）
...
*VeritasChain Protocol (VCP) 规范 v1.2 结束*
